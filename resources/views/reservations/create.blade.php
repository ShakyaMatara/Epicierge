            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                <form method="POST" action="{{ route('reservations.store') }}">
                    ...
                    <div class="mt-4">
                        ...
                        <textarea id="reservation_description" name="reservation_description" class="block mt-1 w-full" rows="3" required>{{ old('reservation_description') }}</textarea>
                    ...
                    <div class="mt-4">
                        ...
                        <x-input id="reservation_time" class="block mt-1 w-full" type="time" name="reservation_time" :value="old('reservation_time')" required />
                        <x-input-error for="reservation_time" class="mt-2" />
                    </div>
                    <!-- Reservation Guests -->
                    <div class="mt-4">
                        <x-label for="reservation_guests" value="{{ __('Reservation Guests') }}" />
                        <x-input id="reservation_guests" class="block mt-1 w-full" type="number" name="reservation_guests" :value="old('reservation_guests')" required />
                    ...
                    <div class="mt-4">
                        ...
                        <select id="reservation_status" name="reservation_status" class="block mt-1 w-full" required>
                            ...
                            <option value="pending" @selected(old('reservation_status') == 'pending')>Pending</option>
                            <option value="confirmed" @selected(old('reservation_status') == 'confirmed')>Confirmed</option>
                            <option value="cancelled" @selected(old('reservation_status') == 'cancelled')>Cancelled</option>
                        </select>
